public class LowerBoundHeapRanked implements Runnable {
    ...
    public LowerBoundHeapRanked(int n, ColEdge[] e, int[][] adjacency) {
        this.nodes = n;
        this.e = e;
    ...
    public void run() {
        ...
        //TODO build ranking here *cries in inevitable recursion*

        int[][] rankingOrder = new int[nodes][2];

        for (int i = 0; i < executionOrder.length; i++) {
            int score = 0;
            for (int j = 0; j < nodes; j++) {
                if (adjacency[executionOrder[i][0]][j] == 1) {
                    //for(int k = 0; k < nodes; k++) score += adjacency[i][k];
                    score += executionOrder[j][1];
                }
            }
            rankingOrder[i][0] = executionOrder[i][0];
            rankingOrder[i][1] = score;
        }

        HeapSort.sort(rankingOrder);

        if(Config.DEBUG)
            for (int i = 0; i < 10; i++)
                System.out.println(executionOrder[i][0] + " : " + executionOrder[i][1] + "; " + rankingOrder[i][0] + " : " + rankingOrder[i][1]);

        //restored from older version
        for (int i = 0; i < neighbours.length; i++) {
            max = 0;
            if (rankingOrder[i][1] > lowerBound + 1) {    //+1 for the i=j 0s
                max = recursiveSearch(e, adjacency, neighbours[rankingOrder[i][0]], lowerBound);
                if (max > lowerBound) {
                    lowerBound = max;
                    whereis = i;
                }
    ...
    private static int recursiveSearch( ColEdge[] e, int[][] adjacent, int[] neighbours, int depth){
        if(neighbours.length > depth){
            ...
            if(sum == ( (adjacentNeighbours.length) * (adjacentNeighbours.length-1) ) ){
				...
				*/
                if(Config.DEBUG)System.out.println("Branch 2 Success with lowerbound of " + (adjacentNeighbours.length - 1) );
